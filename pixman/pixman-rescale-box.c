...
PIXMAN_EXPORT
int downscale_box_filter(const struct rescale_fetcher *fetcher, void *closure, unsigned orig_width, unsigned orig_height,
		...
		uint32_t *dest, int dst_stride)
{
    ...
    */
    ...
    uint32_t *temp_buf = pixman_malloc_abc ((orig_height + scaled_height-1)/scaled_height+1, scaled_width, sizeof(uint32_t));
